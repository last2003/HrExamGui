/*
 * Surge request script for PHS mini-program traffic.
 * Captures key request data and forwards it to a Windmill/HTTP receiver.
 * Configure with: script-path=phs_capture_surge.js, argument=endpoint=...&token=...
 */

const SCRIPT_NAME = 'phs-capture-surge';
...
function postCapture(endpoint, token, payload, doneOnce) {
  ...
  $httpClient.post({
    ...
    if (error) {
      console.log(`[${SCRIPT_NAME}] post failed: ${error}`);
    } else {
      const status = response && response.status ? response.status : 'unknown';
      console.log(`[${SCRIPT_NAME}] saved status=${status} token=${mask(payload.token)} response=${String(data || '').slice(0, 120)}`);
    }
    doneOnce();
  });
}
